...
import { Employee, EmployeePreference, AvailabilityStatus, TimeSlotPreference, AuthUser, PublishedSchedule } from '../types';
import { PencilSquareIcon, CheckCircleIcon, CalendarIcon, UserCircleIcon } from '@heroicons/react/24/outline';
import { CalendarDaysIcon as SolidCalendarIcon } from '@heroicons/react/24/solid';
import TimeSlotModal from './TimeSlotModal';

interface EmployeeViewProps {
  loggedInUser: AuthUser;
  loggedInEmployeeDetails: Employee;
...
const EmployeeView: React.FC<EmployeeViewProps> = ({
  ...
  const myShifts = useMemo(() => {
    ...
    const allShifts = Object.entries(publishedSchedule.schedule)
      .filter(([key]) => key !== 'unassigned_shifts')
      .flatMap(([date, shifts]) => 
        shifts.map(shiftText => ({ date, shiftText }))
      );

    return allShifts
      .filter(shift => shift.shiftText.startsWith(loggedInUser.name))
      .sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());

  }, [publishedSchedule, loggedInUser.name]);
    ...
    <div className="space-y-8">
        <section className="p-6 bg-gray-50 rounded-xl shadow-lg border border-gray-200">
            <h2 className="text-2xl font-semibold text-gray-800 mb-4 flex items-center">
                <SolidCalendarIcon className="h-7 w-7 mr-3 text-gray-700" />
                確定シフト ({publishedSchedule ? publishedSchedule.month : '未公開'})
            </h2>
            {publishedSchedule ? (
                myShifts.length > 0 ? (
                    <ul className="space-y-3">
                        {myShifts.map(({ date, shiftText }, index) => (
                            <li key={index} className="p-3 bg-white border border-gray-200 rounded-lg shadow-sm">
                                <p className="font-semibold text-gray-800">
                                    {new Date(date + "T00:00:00").toLocaleDateString('ja-JP', { month: 'long', day: 'numeric', weekday: 'long' })}
                                </p>
                                <p className="text-gray-700">{shiftText.substring(loggedInUser.name.length + 2)}</p>
                            </li>
                        ))}
                    </ul>
                ) : (
                    <p className="text-gray-500">あなたに割り当てられたシフトはありません。</p>
                )
            ) : (
                <p className="text-gray-500">現在、公開されているシフトはありません。</p>
            )}
        </section>

        <div className="space-y-6 p-2 sm:p-0">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 pb-4 border-b border-gray-200">
                <h2 className="text-2xl font-semibold text-gray-800 mb-2 sm:mb-0">シフト希望を提出</h2>
                <div className="p-3 bg-gray-100 rounded-md text-sm text-gray-700 flex items-center border border-gray-200 self-start sm:self-center">
                    ...
                    {/* --- ここから変更 --- */}
                    <span><strong className="text-black">{loggedInEmployeeDetails.name}</strong></span>
                    {/* --- ここまで変更 --- */}
                </div>
            </div>
